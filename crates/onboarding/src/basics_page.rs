...
fn render_zed_agent_button(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    ...
    let _is_free = matches!(plan, Some(Plan::ZedFree) | None);
    let _is_pro = matches!(plan, Some(Plan::ZedPro));
    let _is_trial = matches!(plan, Some(Plan::ZedProTrial));
    ...
    let _is_signing_in = status.is_signing_in();
    let _is_signed_in = !is_signed_out;
